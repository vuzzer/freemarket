  ...
  PrimaryCryptoBloc({required this.primaryCryptoUsecase})
    ...
    on<PrimaryCryptoEvent>(
      (event, emit) async {
        await event.map(getPrimaryCrypto: (e) async {
        ...
        }, changePrimaryCrypto: (value) async {
          emit(state.copyWith(loading: true));

          final primarCrypto =
              await primaryCryptoUsecase.changePrimaryCrypto(value.crypto);
          ...
          primarCrypto.fold(
              ...
              (crypto) => emit(state.copyWith(
                  ...
                  crypto: crypto,
                  successOrFailure: right(Success()))));
        }, removePrimaryCrypto: (value) async {
          emit(state.copyWith(loading: true));

          final primarCrypto =
              await primaryCryptoUsecase.removePrimaryCrypto(value.crypto);
